<?php
    ...
    $event_result = mysqli_query($connection, "SELECT * FROM `events` WHERE `id` = '{$_GET['event_id']}'");
    $event = mysqli_fetch_assoc($event_result);

    $session_result = mysqli_query($connection, "SELECT * FROM `sessions` WHERE `id` = '{$_GET['id']}'");
    ...
    if (isset($_POST['submit'])) {
        ...
        if ($error == false) {
            mysqli_query($connection, "UPDATE `sessions` SET `room_id`='$room_id', `title`='$title', `description`='$description', `speaker`='$speaker', `start`='$start', `end`='$end', `type`='$type', `cost`='$cost' WHERE `id`={$session['id']}");

            header('Location: ../events/detail.php?id='.$event['id'].'&session_update=true#sessions');
        }
    }
?>
            ...
            <form class="needs-validation" method="POST" novalidate action="">
                ...
                <div class="row">
                    <div class="col-12 col-lg-4 mb-3">
                        ...
                        <input type="text" class="form-control <?php if ($title_err) {echo('is-invalid');} ?>" id="inputTitle" name="title" placeholder="" value="<?php echo($session['title']); ?>">
                ...
                <div class="row">
                    <div class="col-12 col-lg-4 mb-3">
                        ...
                        <input type="text" class="form-control <?php if ($speaker_err) {echo('is-invalid');} ?>" id="inputSpeaker" name="speaker" placeholder="" value="<?php echo($session['speaker']); ?>">
                        <div class="invalid-feedback">
                        <?php if ($speaker_err) {
                                echo $speaker_err;
                            } ?>
                        ...
                        <select class="form-control" id="selectRoom" name="room">
                            <?php
                                ...
                                while ($row = mysqli_fetch_assoc($rooms_result)) {
                                    ...
                                    echo '
                                        <option value="'.$id.'" '.$selected_room.'>'.$name.' / '.$channel['name'].'</option>
                                    ';
                                }
                            ?>
                        </select>
                ...
                <div class="row">
                    <div class="col-12 col-lg-4 mb-3">
                        ...
                        <input type="number" class="form-control" id="inputCost" name="cost" placeholder="" value="<?php if ($session['cost']) {echo($session['cost']);} else {echo(0);} ?>">
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-12 mb-3">
                        ...
                        <textarea class="form-control <?php if ($description_err) {echo('is-invalid');} ?>" id="textareaDescription" name="description" placeholder="" rows="5"><?php echo($session['description']); ?></textarea>
                        <div class="invalid-feedback">
